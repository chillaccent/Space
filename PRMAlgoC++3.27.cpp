...
typedef boost::adjacency_list<boost::listS, boost::vecS, boost::undirectedS, boost::property<boost::vertex_name_t, vector<double> >, boost::property<boost::edge_weight_t, double> > Graph;
...
double distance(const Graph& graph, Vertex v1, Vertex v2) {
    const vector<double> p1 = boost::get(boost::vertex_name, graph)[v1];
    const vector<double> p2 = boost::get(boost::vertex_name, graph)[v2];
...
vector<Vertex> prm(const ProblemSpace& problem_space, double distance_threshold, const vector<vector < double > > & puzzle_piece_positions) {
    ...
    for (auto pos : puzzle_piece_positions) {
        ...
        boost::put(boost::vertex_name, graph, v, pos);
...
for (int i = 0; i < puzzle_piece_vertices.size(); i++) {
    for (int j = i + 1; j < puzzle_piece_vertices.size(); j++) {
        double d = euclidean_distance(boost::get(boost::vertex_name, graph, puzzle_piece_vertices[i]), boost::get(boost::vertex_name, graph, puzzle_piece_vertices[j]));
...
for (int i = 0; i < cubesat_vertices.size(); i++) {
    for (int j = 0; j < puzzle_piece_vertices.size(); j++) {
        double d = euclidean_distance(boost::get(boost::vertex_name, graph, cubesat_vertices[i]), boost::get(boost::vertex_name, graph, puzzle_piece_vertices[j]));
        if (d <= distance_threshold) {
            Edge e;
            bool success;
...
vector<Vertex> find_path(Graph& graph, const vector<Vertex>& puzzle_piece_vertices, const vector<Vertex>& cubesat_vertices, const ProblemSpace& problem_space, double distance_threshold) {
    ...
    for (int i = 0; i < 1000; ++i) { // Sample 1000 candidate nodes
        ...
        for (auto pp_v : puzzle_piece_vertices) {
            double dist = euclidean_distance(boost::get(boost::vertex_name, graph, v), boost::get(boost::vertex_name, graph, pp_v));
        ...
        for (auto cs_v : cubesat_vertices) {
            double dist = euclidean_distance(boost::get(boost::vertex_name, graph, v), boost::get(boost::vertex_name, graph, cs_v));
            if (dist <= distance_threshold) {
                Edge e;
                bool success;
...
int main() {
    ...
    problem_space.cubesat_positions = {
        {0, 0, 0},
        {50, 50, 50}
    };
    problem_space.puzzle_piece_positions = {
        {10, 10, 10},
        {20, 20, 20},
        {30, 30, 30},
        {40, 40, 40}
    ...
    for (const auto& pos : problem_space.cubesat_positions) {
        cubesat_vertices.push_back(boost::add_vertex(pos, graph));
    }
    ...
    if (path.empty()) {
        ...
        for (const Vertex& v : path) {
            cout << "[" << graph[v].name[0] << ", " << graph[v].name[1] << ", " << graph[v].name[2] << "]" << endl;
        }
    }
