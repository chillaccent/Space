...
double objective_function(const ProblemSpace& problem_space, const vector<Vertex>& path){
    double cost = 0.0;
    for (int i = 0; i < path.size() - 1; i++) {
        Vertex v1 = path[i];
        Vertex v2 = path[i+1];
        cost += distance(problem_space.puzzle_piece_positions[graph[v1].pos],
                         problem_space.puzzle_piece_positions[graph[v2].pos]);
    }
    return cost;
}
...
int main() {
    ...
    problem_space.L = 10;
    problem_space.puzzle_piece_positions = { {1, 2, 3}, {4, 5, 6}, {7, 8, 9} };
    problem_space.cubesat_positions = { {0, 0, 0}, {10, 10, 10} };

    double distance_threshold = 5.0;

    vector<vector<double> > puzzle_piece_positions = { {1, 2, 3}, {4, 5, 6}, {7, 8, 9} };

    vector<Vertex> path = prm(problem_space, distance_threshold, puzzle_piece_positions);
